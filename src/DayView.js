...
export default class DayView extends React.PureComponent {
  ...
  componentDidMount () {
    ...
    this.props.onDateChange && this.props.onDateChange(this.props.date)
  }
  ...
  _renderLines () {
    ...
    return range(0, 25).map((item, i) => {
      ...
      if (i === 0) {
        ...
        timeText = !formatTime ? `${i} ÖS` : `${i} ÖS`
      } else if (i === 24) {
        timeText = !formatTime ? `12 ÖÖ` : `12 ÖÖ` 
      } else {
        timeText = !formatTime ? `${i - 12} ÖS` : `${i - 12} ÖS`
      }
